...
ALTER TABLE documents_ordre_mission ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Accès aux documents des ordres de mission"
  ON documents_ordre_mission FOR SELECT
  USING (
    EXISTS (
      SELECT 1 FROM ordres_mission
      WHERE ordres_mission.id = documents_ordre_mission.id_ordre_mission
      AND (
...
-- Validations ordre mission
ALTER TABLE validations_ordre_mission ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Accès aux validations des ordres de mission"
  ON validations_ordre_mission FOR SELECT
  USING (
    EXISTS (
      SELECT 1 FROM ordres_mission
      WHERE ordres_mission.id = validations_ordre_mission.id_ordre_mission
      AND (
        ordres_mission.id_demandeur = auth.uid()
        OR EXISTS (
          SELECT 1 FROM hierarchies
